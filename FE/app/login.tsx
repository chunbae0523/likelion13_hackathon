...
import {
  registerProfile,
  checkUserLoginByEmail,
  fetchUserProfiles,
} from "../src/api/loginApi";
import { saveUUID, getUUID } from "../src/utils/handleAsyncUUID";
...
export default function AuthScreen() {
  ...
  const [isAutoLogin, setIsAutoLogin] = useState(false); // 로그인 상태 유지 토글
  ...
  const handleLogin = async () => {
    ...
    if (data) {
      // true면 성공
      await saveUUID(data.id);
      await AsyncStorage.setItem("autoLogin", isAutoLogin ? "true" : "false");
      router.push("/tabs/home");
      Alert.alert("로그인 성공", `환영합니다, ${data.username}님!`);
    } else {
      Alert.alert(
        "로그인 실패",
        `잘못된 비밀번호 또는 존재하지 않는 이메일입니다.`
      );
    }
    setLoading(false);
  };
